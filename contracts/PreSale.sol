...
import { ILockup, ISubscription } from "./ILockup.sol";
...
contract PreSale is Rounds, ReentrancyGuardTransient {
    using SafeERC20 for IERC20;
    using Address for address payable;

    /// @custom:price The price of token from price feed
    /// @custom:normalizationFactorForToken The normalization factor to achieve return value of 18 decimals ,while calculating token purchases and always with different token decimals
    struct TokenInfo {
        uint256 latestPrice;
        uint8 normalizationFactorForToken;
    }

    /// @custom:projectAmount The amount tansferred to project wallet
    /// @custom:platformAmount The amount tansferred to platform wallet
    /// @custom:burnAmount The amount tansferred to burn wallet
    /// @custom:equivalence The amount tansferred to claims contract
    struct TransferInfo {
    ...
    uint256 public DISCOUNT_PPM = 200_000; // need to create setter function
    ...
    /// @notice Thrown when there is a Gems round and user tries to purchase with other Tokens
    error PurchaseOnlyAllowedWithGems();
    ...
    /// @notice Updates the gems round limit
    /// @param newLimit The new gems round limit in USD
    ...
    /// @notice Updates the lower limit for purchase
    /// @param newLimit The new lower limit in usd
    ...
    /// @notice Updates the upper limit for purchases
    /// @param newLimit The new upper limit in usd
    ...
    /// @notice Updates the discount percentage for purchases
    /// @param newDiscount The new discount percentage in PPM (parts per million)
    function setDiscountForUpperLimit(uint256 newDiscount) external onlyOwner {
        if (DISCOUNT_PPM == newDiscount) {
            revert IdenticalValue();
        }
    ...
    function purchaseTokenWithETH(
        ...
        if (rounds[round].isGemsRound) {
            revert PurchaseOnlyAllowedWithGems();
        }
        ...
        string memory roundType = _getRoundType(round);
    ...
    function getLatestPrice(IERC20 token) public view returns (TokenInfo memory) {
        ...
        return
            TokenInfo({ latestPrice: uint256(price), normalizationFactorForToken: data.normalizationFactorForToken });
    }
    ...
    /// @dev Returns the type of a round based on its data
    function _getRoundType(uint32 round) internal view returns (string memory) {
        RoundData memory data = rounds[round];
    ...
    function _getRoundPriceForToken(
        ...
        (uint256 v1, uint256 v2) = subscription.endTimes(user);
